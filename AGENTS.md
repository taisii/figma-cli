...
- `src/llm_client.py`: 会話サマリーで使用する Gemini クライアントの初期化を共通化します。`build_generative_model` を介してモデルを取得してください。
...
## コーディングスタイルと命名規則
Python は 4 スペースインデント、必要に応じて型ヒントを追加します。モジュールはスネークケース、クラスはパスカルケースに統一し、副作用を持つロジックは極力薄いラッパーに留めてテスタブルな関数へ分解してください。Gemini を利用する処理（会話ログの要約など）は `llm_client.build_generative_model` を経由してください。
...
## セキュリティと設定上の注意
Gemini API キーは `.env` に保存し、履歴やレビューコメントに含めないでください（会話サマリー機能で利用します）。`config.yaml` は小さな値でも変更したらドキュメントに反映し、再現性を保ちます。生成された Markdown には機密情報が含まれる可能性があるため、共有時は必要に応じて編集した上で配布してください。
