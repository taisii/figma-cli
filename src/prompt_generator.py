...
"""
    ...
    @staticmethod
    def generate_daily_report_prompt(diff_summary, outline):
        prompt = f"""
# 指示

以下のコンテキスト情報を基に、日報を生成してください。
日報には、今日の作業内容、それによって更新されたアウトライン、そして将来の調査の種となる「小さな疑問」を必ず含めてください。
...
今日の作業で感じた小さな疑問や不確かな点を具体的に記述してください。
また、その疑問に至った背景や文脈も併せて説明してください。これは将来の戦略立案のための重要な情報となります。

## ネクストアクション

明日以降に取り組むべき具体的なタスクを記述してください。
"""
        return prompt

    @staticmethod
    def generate_strategy_prompt(outline_content, all_logs_content):
        prompt = f"""
# 指示

以下の情報を基に、今後の研究の方向性を定めるための戦略提案書を生成してください。
...
## 調査の背景
（ここに、アウトラインとログを要約し、現在の研究の全体像と文脈を記述）

## 未解決の問い (Questions)
（ここに、日報の Unresolved_Points などを分析し、深掘りすべき具体的な問いをリストアップ）

## 生成されるべき仮説 (Hypotheses)
（ここに、上記の問いに対する仮説を複数生成）

## 調査すべきキーワード (Keywords)
（ここに、仮説を検証するために調査すべき具体的なキーワードをリストアップ）
"""
        return prompt